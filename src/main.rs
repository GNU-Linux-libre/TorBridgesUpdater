...
/* Initialize windows start */
                ...
                mainwindow.connect_hide(clone!(@strong app_settings, @weak app, @weak settingswindow, @weak captchawindow, @weak bridgeswindow, @weak qrcodewindow => move |_| {
                    app_settings.set_property("backgroundmode", true);

                    settingswindow.destroy();
                    captchawindow.destroy();
                    bridgeswindow.destroy();
...
/* Settings window start */
                settings_button.connect_clicked(clone!(@strong app_settings, @strong app_settings_backup, @weak days_entry, @weak hours_entry, @weak minutes_entry, @weak seconds_entry, @weak updbrdgs, @weak torrcfileentry, @weak torrcbutton, @weak disableold, @weak savbrdgs, @weak bridgesfileentry, @weak bridgesbutton, @weak keepold, @weak bridgetype_button, @weak checkipv6, @weak useproxy, @weak proxytype_button, @weak proxy_host, @weak proxy_port, @weak getonion, @weak shownotifications, @weak runinbackground, @weak settings_apply, @weak label_days, @weak label_hours, @weak label_minutes, @weak label_seconds, @weak settingswindow, @weak gridtimer => move |_| {
                    app_settings_backup.load_from(&app_settings);
...
/* QR code window end */
                ...
                action_close.connect_activate(clone!(@weak app, @strong app_settings, @weak mainwindow, @weak settingswindow, @weak captchawindow, @weak bridgeswindow, @weak qrcodewindow, @weak aboutwindow => move |_, _| {
                    ...
                    if settingswindow.is_active() && settingswindow.is_visible() {
                        settingswindow.hide();
                    }
                    if captchawindow.is_active() && captchawindow.is_visible() {
                        captchawindow.hide();
                    }
                    if bridgeswindow.is_active() && bridgeswindow.is_visible() {
                        bridgeswindow.hide();
                    }
                    if qrcodewindow.is_active() && qrcodewindow.is_visible() {
                        qrcodewindow.hide();
                    }
                    if aboutwindow.is_active() && aboutwindow.is_visible() {
                        aboutwindow.hide();
                    }
                }));
                ...
                channeltimer.1.attach(None, clone!(@weak app, @weak label_timer, @weak label_timer_until, @weak progressload, @weak mainwindow, @weak settingswindow, @weak captchawindow, @weak bridgeswindow, @weak qrcodewindow, @weak aboutwindow, @strong app_settings => @default-return glib::Continue(true), move |_| {
                    ...
                    if now_time >= next_time {
                        ...
                        if (!mainwindow.is_visible() || !mainwindow.is_active()) && (!settingswindow.is_visible() || !settingswindow.is_active()) && (!captchawindow.is_visible() || !captchawindow.is_active()) && (!bridgeswindow.is_visible() || !bridgeswindow.is_active()) && (!qrcodewindow.is_visible() || !qrcodewindow.is_active()) && (!aboutwindow.is_visible() || !aboutwindow.is_active()) {
